    ...
    <>
      <div className="mt-20 pt-10 pb-14 bg-stone-800 rounded-2xl border flex flex-col items-center">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="lg:text-right text-center lg:ps-10 lg:pe-10 my-auto">
            ...
            <ul className="text-lg 2xl:text-xl text-stone-400 space-y-2">
              {contactDetails.map((link, index) => (
                <li key={index} className="hover:text-sky-900">
                  <Link href={link.href}>{link.text}</Link>
                </li>
              ))}
            </ul>
          </div>
          <div className="flex justify-center items-center pt-10 md:ml-10 lg:pt-0">
            <Image
              src={logo}
              width={275}
              height={85}
          ...
          <div className="lg:text-left text-center lg:ps-20 my-auto">
            <h2 className="text-2xl 2xl:text-4xl text-amber-600 font-bold pb-6 md:pt-20 md:mt-2">
              Navigation
            </h2>

            <ul className="text-lg 2xl:text-xl text-stone-400 space-y-2">
              {navLinks.map((link, index) => (
                <li key={index} className="hover:text-amber-600">
                  <Link href={link.href}>{link.text}</Link>
                </li>
              ))}
            </ul>
